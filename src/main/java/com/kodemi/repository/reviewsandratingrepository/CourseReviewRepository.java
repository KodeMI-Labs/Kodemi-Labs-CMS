...
public class CourseReviewRepository {
	...
	/**
	 * Query by course_id using the GSI "course-id-index". Much more efficient than
	 * full scan — only returns reviews for the given course.
	 */
	public List<CourseReview> findByCourseId(String courseId) {
		...
		DynamoDBQueryExpression<CourseReview> queryExpression = new DynamoDBQueryExpression<CourseReview>()
				.withIndexName("course-id-index").withConsistentRead(false) // GSI only supports eventual consistency
